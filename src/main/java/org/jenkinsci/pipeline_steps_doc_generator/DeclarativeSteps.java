...
public class DeclarativeSteps {
    ...
    private static final List<String> blockedOptionSteps =
            Arrays.asList("node", "stage", "withEnv", "script", "withCredentials");
    private static final List<Class<?>> blockedOptionStepContexts =
            Arrays.asList(Launcher.class, FilePath.class, Computer.class);
    ...
    public void generateDeclarativeAscii(String declarativeDest, HyperLocalPluginManager pluginManager) {
        ...
        for (Map.Entry<String, List<Class<? extends Descriptor>>> entry :
                getDeclarativeDirectives().entrySet()) {
    ...
    private Map<String, List<Descriptor>> processDescriptors(
            @NonNull Class<? extends Descriptor> c,
        ...
        List<? extends Descriptor> descriptors =
                pluginManager.getPluginStrategy().findComponents(c);
    ...
    private Map<String, List<Class<? extends Descriptor>>> getDeclarativeDirectives() {
        ...
        directives.put(
                "options",
                Arrays.asList(JobPropertyDescriptor.class, DeclarativeOptionDescriptor.class, StepDescriptor.class));
